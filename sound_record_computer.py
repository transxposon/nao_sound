...
def record_audio(file_name, device, sample_rate=16000):
    ...
    try:
        ...
        #  recording
        # the thing is with sd.rec() you need to preallocate a duration
        # here the default is an hour
        # i record for two minutes and then crop the actual elapsed time from that file otherwise
        # even a 10 seconds file would be an hour long.
...
if __name__ == "__main__":
    ...
    try:
        device_index = int(raw_input("Enter the device index for recording: "))
        file_name = raw_input("Enter the desired file name (without .wav extension): ")
